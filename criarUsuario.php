...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $prontuario = $_POST['prontuario_usuario'];
    $password = $_POST['senha_usuario'];
    $nome = $_POST['nome_usuario'];
    $linha = $_POST['linha_usuario'];
    // $linha = $_POST['tipo_usuario'];

    // Prepare a consulta SQL para evitar SQL injection
    $sql = "INSERT INTO usuario (prontuario_user, name_user, password_user, linha) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    
    // Verifica se a preparação foi bem-sucedida
    if ($stmt) {
        // Use 'ssss' se todos os campos forem strings
        $stmt->bind_param('ssss', $prontuario, $nome, $password, $linha);
        $stmt->execute();
...
select{
    ...
    height: 50px;
    width: 100%;
    background-color: rgb(76 76 76 / 94%);
    border-radius: 3px;
    padding: 0 10px;
    margin-top: 8px;
    font-size: 14px;
