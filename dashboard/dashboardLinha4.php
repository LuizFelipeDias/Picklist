...
<script>
    ...
    // Carregar o som
    const notificationSound = new Audio('assets/audio/picklist-liberada.mp3')

    function alertNovaOrdem() {
        console.log('Função alertNovaOrdem chamada'); // Adicione esta linha
        var alertSound = new Audio('assets/audio/picklist-liberada.mp3');
        alertSound.play();
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.data1.forEach(function (item) {
                    console.log(item)
                    if(item.status == 4 ){
                        status1Table.append(`<tr class="cancelada">
                            <td>${item.barcode}</td>
                        </tr>`);
                    }else{
